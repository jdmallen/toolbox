...
/// <remarks>
///   <para>
///     This class extends <see cref="ScopedBackgroundService{TService}" /> to provide cron-based
///     scheduling using the NCrontab library. Work executes only when the current time matches
///     or exceeds the next scheduled occurrence according to the cron expression.
///   </para>
///   <para>
///     The <see cref="ScopedBackgroundService{TService}.LoopDelay" /> property controls how
///     frequently the time is checked (not when work executes). A shorter delay provides more
...
public abstract class ScheduledBackgroundService<TService> : ScopedBackgroundService<TService>
{
	...
	/// <summary>
	///   Initializes a new instance of the <see cref="ScheduledBackgroundService{TService}" /> class.
	/// </summary>
	...
	///   The cron expression defining when the service should execute. If invalid, defaults to
	///   "*/1 * * * *" (every minute) and logs an error.
	/// </param>
	/// <param name="includeSeconds">
	///   If true, the cron expression includes seconds as the first field (6 fields total).
	...
	///   </para>
	///   <para>
	///     Extended format with seconds (6 fields): second minute hour day month day-of-week
	///   </para>
	///   <para>
	///     Examples:
	///     <list type="bullet">
	///       <item><description>"0 * * * *" - Every hour at minute 0</description></item>
	///       <item><description>"*/15 * * * *" - Every 15 minutes</description></item>
	///       <item><description>"0 2 * * *" - Daily at 2:00 AM</description></item>
	///       <item><description>"0 0 * * 0" - Weekly on Sunday at midnight</description></item>
	///       <item><description>"*/30 * * * * *" - Every 30 seconds (with includeSeconds: true)</description></item>
	///     </list>
	///   </para>
	/// </remarks>
		...
		: base(logger, scopeFactory, timeProvider)
	{
		...
		{
			throw new ArgumentException("Cron schedule cannot be null or whitespace.", nameof(cronSchedule));
		}
	...
	/// </summary>
	/// <remarks>
	///   This property is updated after each execution to reflect the next scheduled occurrence
	///   according to the cron expression. It can be useful for diagnostics or logging.
	/// </remarks>
	protected DateTime NextRunTime { get; private set; }

	/// <summary>
	///   Checks if the current time has reached or passed the next scheduled execution time,
	///   and if so, executes the work and updates the next run time.
	/// </summary>
	/// <param name="stoppingToken">The cancellation token.</param>
	/// <returns>A task representing the asynchronous operation.</returns>
	/// <remarks>
	///   This method is called repeatedly according to <see cref="ScopedBackgroundService{TService}.LoopDelay" />.
	///   Work only executes when the current time matches or exceeds <see cref="NextRunTime" />.
	/// </remarks>
