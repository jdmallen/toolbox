...
/// <remarks>
///   <para>
///   This class provides a foundation for long-running background tasks that
///   need to execute
///   repeatedly at regular intervals. It handles the common concerns of scoped
///   service
...
public abstract class ScopedBackgroundService<TService> : BackgroundService
{
	...
	/// <remarks>
	///   <para>
	///   For direct subtypes of this class, this represents the time to wait between
	///   the
	///   completion of one execution and the start of the next.
	///   </para>
	///   <para>
	///   For <see cref="ScheduledBackgroundService{TService}" />, this represents
	///   how frequently
	///   the time is checked to determine if the next scheduled occurrence should
	///   execute.
	///   </para>
	/// </remarks>
	...
	/// </summary>
	/// <remarks>
	/// Defaults to <see cref="Hosting.OverlapBehavior.AllowOverlap" /> for backward
	/// compatibility.
	/// Override this property to change the overlap behavior.
	/// </remarks>
	protected virtual OverlapBehavior OverlapBehavior =>
		OverlapBehavior.AllowOverlap;
	...
	/// <summary>
	/// This method is called when the <see cref="IHostedService" /> starts and
	/// returns a task
	/// that represents the lifetime of the long running operation(s) being
	/// performed.
	/// </summary>
	...
	/// <remarks>
	///   <para>
	///   This implementation creates a loop that repeatedly executes work within a
	///   scoped
	///   context, with error handling and logging for each iteration. The loop
	///   continues
	///   until the stopping token is cancelled.
	///   </para>
	///   <para>
	///   Each execution is wrapped in error handling to prevent a single failure
	///   from
	///   stopping the entire service. Unhandled exceptions are logged but do not
	///   propagate.
	///   </para>
	/// </remarks>
	protected override async Task ExecuteAsync(CancellationToken stoppingToken)
	{
		do
		{
			...
			if (OverlapBehavior != OverlapBehavior.AllowOverlap)
			{
				...
				var previousValue = Interlocked.CompareExchange(ref _isExecuting, 1, 0);
	...
	/// <see cref="ExecuteInScopeAsync(IServiceScope, CancellationToken)" />
	/// within that scope.
	/// </summary>
	/// <param name="stoppingToken">The cancellation token.</param>
	/// <returns>A task representing the asynchronous operation.</returns>
	/// <remarks>
	/// Override this method if you need custom scope creation logic. The default
	/// implementation
	...
	///   background work. It is called repeatedly according to
	///   <see cref="LoopDelay" />
	///   and the configured <see cref="OverlapBehavior" />.
	///   </para>
	///   <para>
	///   The <paramref name="scope" /> parameter provides access to scoped services
	///   through
	///   its <see cref="IServiceScope.ServiceProvider" />. This ensures proper
	///   lifetime
	...
	///   allowing
	///   the service to continue running. If you need different error handling
	///   behavior,
