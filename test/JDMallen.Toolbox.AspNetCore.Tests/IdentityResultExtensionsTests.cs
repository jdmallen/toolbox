...
public class IdentityResultExtensionsTests
{
	private static IdentityResult FailedResult() =>
		IdentityResult.Failed(
			...
			new IdentityError { Code = "PasswordTooShort", Description = "Need 8+ chars." },
			new IdentityError { Code = "DuplicateEmail", Description = "Email in use." });

	[Fact]
	public void ToErrorDictionary_GroupsDescriptionsByErrorCode()
	{
		var errors = FailedResult().ToErrorDictionary();
	...
	{
		var expected = Results.Ok("created");

		var actual = IdentityResult.Success.Match(() => expected);
	...
	{
		var actual = FailedResult().Match(() => Results.Ok("created"));
	...
	public async Task MatchAsync_ReturnsSuccessResultWhenSucceeded()
	{
		...
		var actual = await IdentityResult.Success.Match(
	...
	{
		var actual = await FailedResult().Match(
			() => Task.FromResult<IResult>(Results.Ok("created")));
